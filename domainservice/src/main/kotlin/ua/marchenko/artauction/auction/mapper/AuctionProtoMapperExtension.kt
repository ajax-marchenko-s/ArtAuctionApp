...
fun MongoAuction.toAuctionProto(): AuctionProto {
    return AuctionProto.newBuilder().also {
        ...
        it.startedAt = (startedAt ?: LocalDateTime.MIN).toTimestampProto()
        it.finishedAt = (finishedAt ?: LocalDateTime.MIN).toTimestampProto()
        it.addAllBuyers(buyers?.map { bid -> bid.toBidProto() }.orEmpty())
    }.build()
}

fun AuctionProto.toMongoAuction(): MongoAuction {
    return MongoAuction(
        id = id.toObjectId(),
